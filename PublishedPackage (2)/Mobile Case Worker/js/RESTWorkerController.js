...
(function(Core, Platform) {"use strict";
    ...
    var ForbiddenFieldNames = ["", "_id", AppMagic.Constants.Services.SpIdProperty],
        ...
        RESTWorkerController = Core.Class.define(function RESTWorkerController_ctor(disp, def, multipartFormDataHelper) {
            this._disp = disp;
            this._def = def;
            this._multipartFormDataHelper = multipartFormDataHelper;
            this._typesByPrefixAndName = new AppMagic.Services.TypesByPrefixAndName(this._def.typesbyprefix)
        }, {
            _disp: null, _def: null, _fns: null, _functionDict: null, _multipartFormDataHelper: null, _typesByPrefixAndName: null, _workerHandle: null, _hiddenFunctionDict: null, functions: {get: function() {
                        return this._fns
                    ...
                    this._workerHandle = this._disp.createWorker(["AppMagic", "Worker", "RESTWorker"], [this._def.typesbyprefix], "/js/common/RESTWorker.js")
                }, _isFunctionDefined: function(functionName) {
                    return typeof this._functionDict[functionName] == "object"
                }, _generateFunctions: function() {
                    for (var constants = AppMagic.Constants.Services.Rest, hiddenFns = [], fns = [], funcNames = {}, j, jlen, i = 0, len = this._def.functions.length; i < len; i++) {
                        var functionDef = this._def.functions[i];
                        funcNames[functionDef.name] = !0;
                        var isAuthFunction = functionDef[constants.FunctionKeyIsAuth];
                        var requiredParameters = [],
                            optionalParameters = [],
                            parameters = functionDef.parameters,
                            alreadyDefinedParamNames = {};
                        for (j = 0, jlen = parameters.length; j < jlen; j++) {
                            var parameter = parameters[j],
                                parameterName = parameter[constants.FunctionParameterKeyName];
                            alreadyDefinedParamNames[parameterName] = !0;
                            var arg = {
                                    name: parameterName, dtype: parameter[constants.FunctionParameterKeyDType], options: null, docId: RESTWorkerController.getDocId(parameter)
                                };
                            typeof parameter[constants.DocKey] != "undefined" && (arg[constants.DocKey] = parameter[constants.DocKey]);
                            parameter[constants.FunctionParameterKeyOptions] instanceof Array && (arg.options = parameter[constants.FunctionParameterKeyOptions]);
                            typeof parameter[constants.FunctionParameterKeySampleDefault] != "undefined" && (arg[constants.FunctionParameterKeySampleDefault] = parameter[constants.FunctionParameterKeySampleDefault]);
                            parameter[constants.FunctionParameterKeyRequired] === !0 ? requiredParameters.push(arg) : optionalParameters.push(arg)
                        }
                        if (isAuthFunction)
                            hiddenFns.push({
                                requiredParameters: requiredParameters, optionalParameters: optionalParameters, name: functionDef.name, fn: this._createInvoke(OpNameSendHttpAndParse, requiredParameters, optionalParameters, functionDef)
                            });
                        else {
                            var responseDef = functionDef[constants.FunctionKey_Response];
                            var responseSchema = RESTWorkerController.computeResponseSchema(responseDef, this._typesByPrefixAndName),
                                responseSchemaString = AppMagic.Schema.getSchemaString(responseSchema);
                            fns.push({
                                disableTryIt: functionDef.disabletryit, requiredParameters: requiredParameters, optionalParameters: optionalParameters, name: functionDef.name, schema: responseSchema, schemaString: responseSchemaString, docId: RESTWorkerController.getDocId(functionDef), fn: this._createInvoke(OpNameSendHttpAndParse, requiredParameters, optionalParameters, functionDef), method: functionDef.request.method
                            })
                        }
                    }
                    this._shouldGenerateSignInOutFns() && (this._createSignIn(fns), this._createSignOut(fns));
                ...
                }, _shouldGenerateSignInOutFns: function() {
                    return this._def.auths instanceof Array && this._def.auths.length > 0
                }, _createSignIn: function(fns) {
                    fns.push({
                        requiredParameters: [], optionalParameters: [], description: "Sign in", name: AppMagic.AuthoringTool.Runtime.signInFnName, schema: {type: "b"}, schemaString: "b", fn: function(){}, method: "POST", hideFromBackstage: !0, docId: null
                    })
                }, _createSignOut: function(fns) {
                    fns.push({
                        requiredParameters: [], optionalParameters: [], description: "Sign out", name: AppMagic.AuthoringTool.Runtime.signOutFnName, schema: {type: "b"}, schemaString: "b", fn: function(){}, method: "POST", hideFromBackstage: !0, docId: null
                    })
                    ...
                    return function(functionArguments, auth) {
                            for (var arg, val, parameterMap = {}, i = 0; i < rcount; i++)
                                ...
                                parameterMap[arg.name][ParameterMapItemKeyDType] = arg.dtype;
                            if (ocount > 0) {
                                var opt = functionArguments[rcount] || {};
                                for (i = 0; i < ocount; i++)
                                    arg = optionalParams[i],
                                    val = opt[arg.name],
                                    typeof val != "undefined" && val !== null && (parameterMap[arg.name] = {}, parameterMap[arg.name][ParameterMapItemKeyValue] = val, parameterMap[arg.name][ParameterMapItemKeyDType] = arg.dtype)
                            }
                            var params = {};
                            ...
                            var createRequestAndInvoke = function() {
                                    ...
                                    return that._invokeWorker(op, [params]).then(function(response) {
                                            return RESTWorkerController.postWorkerProcess(functionDef, response, that._typesByPrefixAndName)
                                        }, function(error) {
                                            if (AppMagic.Utility.isCanceledError(error))
                                                throw error;
                                            throw error;
                            ...
                            return bodyDef ? that.buildRequestBodyAsync(bodyDef, parameterMap, params.headers).then(function(response) {
                                ...
                                }) : Core.Promise.wrap().then(createRequestAndInvoke)
                        }
                    ...
                    return Core.Promise.join(promises).then(function(responses) {
                            var failedParameters = [];
                            for (i = 0, len = responses.length; i < len; i++)
                                responses[i].success || failedParameters.push(responses[i].detail);
                            if (failedParameters.length > 0) {
                                var listOfFailed = AppMagic.Utility.formatString(AppMagic.Strings.RestErrorUnableToGetSourcesListFirstItem, failedParameters[0]);
                                for (i = 1, len = failedParameters.length; i < len; i++)
                                    listOfFailed = listOfFailed + AppMagic.Utility.formatString(AppMagic.Strings.RestErrorUnableToGetSourcesListAfterFirstItem, failedParameters[i]);
                                var errorMessage = AppMagic.Utility.formatString(AppMagic.Strings.RestErrorUnableToGetSourcesFormat, listOfFailed);
                                return {
                                        success: !1, message: errorMessage
                                    }
                            }
                            var fmt;
                            return formParameters.length === 0 && (fmt = AppMagic.Utility.formatString("--{0}\r\n\r\n\r\n", boundaryValue), formParameters.push(fmt)), fmt = AppMagic.Utility.formatString("--{0}--\r\n", boundaryValue), formParameters.push(fmt), {
                    ...
                    return RESTWorkerController.getValueAsync(parameter, parameterMap).then(function(response) {
                            ...
                            if (typeof value != "undefined") {
                                ...
                                if (formParameters.push(fmt), paramType === AppMagic.Constants.Services.Rest.XsdType.Base64Binary) {
                                    ...
                                    fmt = AppMagic.Utility.formatString('{0}="{1}"; {2}="{3}"\r\n\r\n', HeaderNameValue, paramName, HeaderFileName, fileName)
                                }
                                else
                                    value = value.toString(),
                                    fmt = AppMagic.Utility.formatString('{0}="{1}"\r\n\r\n', HeaderNameValue, paramName);
            ...
            createValidationResult: function(success, message) {
                ...
                return typeof message != "undefined" && (validationResult.message = message), validationResult
                    ...
                    try {
                        var configResult = doc.parseAndValidateConfig(fileContents, JSON.stringify(templateVariableValues));
                ...
                }, getDocId: function(parentObj) {
                    var constants = AppMagic.Constants.Services.Rest;
                    return typeof parentObj[constants.DocIdKey] != "undefined" ? parentObj[constants.DocIdKey] : null
                    ...
                    var constants = AppMagic.Constants.Services.Rest;
                    var valueDefinition = param[constants.ParamKey_Value],
                        xsdType = param[constants.ParamKey_Type];
                    var value;
                    switch (valueDefinition[constants.ParamValueKey_Type]) {
                        case constants.ParamValueType_Fixed:
                            value = valueDefinition[constants.ParamValueKey_Definition];
                            break;
                        case constants.ParamValueType_Parameter:
                            var parameterDefinition = valueDefinition[constants.ParamValueKey_Definition];
                            if (typeof parameterMap[parameterDefinition] == "undefined")
                                break;
                            var paramInfo = parameterMap[parameterDefinition],
                            ...
                            if (dtype === AppMagic.Schema.TypeImage || dtype === AppMagic.Schema.TypeMedia)
                                return paramValue === "" ? Core.Promise.wrap({
                    ...
                    var constants = AppMagic.Constants.Services.Rest;
                    var valueDefinition = param[constants.ParamKey_Value],
                        paramType = param[constants.ParamKey_Type];
                    var value;
                    switch (valueDefinition[constants.ParamValueKey_Type]) {
                        case constants.ParamValueType_Fixed:
                            value = valueDefinition[constants.ParamValueKey_Definition];
                    ...
                    if (typeof value != "undefined" && paramType === constants.XsdType.String) {
                        ...
                        typeof param[constants.ParamKey_Format] != "undefined" && (value = AppMagic.Utility.formatString(param[constants.ParamKey_Format], value))
                    }
                    return value
                    ...
                    for (var constants = AppMagic.Constants.Services.Rest, params = bodyDef[constants.RequestBodyJsonKey_Params], bodyContainer = {}, j, jlen, i = 0, len = params.length; i < len; i++) {
                        var param = params[i];
                        var path = AppMagic.Utility.parseJsonPointer(param[RequestBodyJsonParamKeyPath]),
                            value = RESTWorkerController.getValue(param, parameterMap);
                        if (typeof value != "undefined")
                            if (path.length === 0)
                                ...
                                var requestBody = bodyContainer.requestBody || {},
                                    context = requestBody;
                                for (j = 0, jlen = path.length - 1; j < jlen; j++) {
                    ...
                    var constants = AppMagic.Constants.Services.Rest;
                    switch (typeDef[constants.TypeDefinitionKey_DefType]) {
                        case constants.DefinitionType_JsonArray:
                            return RESTWorkerController.computeJsonResponseSchema_JsonArray(typeDef, typesByPrefixAndName, maxRepeat, occurrenceTable);
                        case constants.DefinitionType_JsonMapObject:
                    ...
                    if (typeof propertyDef[constants.DefinitionPropertyKey_Type] != "undefined")
                        ...
                        var prefix = typeRef[constants.DefinitionPropertyTypeRefKey_Prefix],
                            ...
                            occurrenceCount = occurrenceTable.getValue(prefix, typeName);
                        if (occurrenceCount = typeof occurrenceCount == "undefined" ? 0 : occurrenceCount, occurrenceCount <= maxRepeat) {
                    ...
                    var constants = AppMagic.Constants.Services.Rest;
                    return AppMagic.Schema.createSchemaForSimple(typeDef[constants.JsonPrimitiveDefinitionKey_DType])
                    ...
                    var constants = AppMagic.Constants.Services.Rest,
                        occurrenceTable = new AppMagic.Utility.TwoKeyDictionary;
                    ...
                    return null
                }, MaxTypeInlineDepth: 5
        });
    Core.Namespace.define("AppMagic.Services.Meta", {RESTWorkerController: RESTWorkerController})
})(WinJS, Windows);