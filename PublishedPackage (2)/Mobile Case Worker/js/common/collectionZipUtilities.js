...
(function(Core, Platform) {"use strict";
    var StringConsts = {
            tagApplicationData: "ApplicationData", tagData: "Data", tagTable: "Table", tagRow: "Row", tagCell: "Cell", tagRecord: "Record", attrName: "ss:Name", attrType: "ss:Type", msImpexAppdataScheme: "ms-appdata:///temp/{0}/Assets/", xmlFileName: "data.xml", schemaFileName: "schema.json", locMapFileName: "locMap.json"
        },
        ImportExportErrors = AppMagic.Constants.ImportExportErrors,
        runtimeConsts = AppMagic.Constants.Runtime;
    function createZip(root) {
        ...
        try {
            ...
            return typeof err != "string" && (err = Strings.ExportUnknownError), Core.Promise.as({error: err})
        }
        ...
        zip.file(StringConsts.locMapFileName, JSON.stringify(root.data));
        ...
        return Core.Promise.join(promises).then(function() {
    ...
    function loadZip(args) {
        try {
            ...
            return Core.Promise.as({error: ImportExportErrors.ImportCorruptedZipError})
        }
        var xmlFile = zip.files[StringConsts.xmlFileName];
        if (typeof xmlFile == "undefined")
            return Core.Promise.as({error: ImportExportErrors.ImportXMLNotFoundError});
        ...
        if (typeof schemaFile != "undefined")
            try {
                ...
                return Core.Promise.as({error: ImportExportErrors.ImportInvalidJSONError})
            }
        ...
        for (var fileName in zip.files) {
            ...
            if (typeof file.options == "undefined" || typeof file.name == "undefined")
                return Core.Promise.as({error: ImportExportErrors.ImportUnkownError});
        ...
        return Core.Promise.join(promises).then(function() {
                try {
                    return xml2json(xmlText, args.impexID, schema)
                }
                catch(error) {
    ...
    function _getFileData(url) {
        try {
            ...
            return Core.Promise.wrapError(ex)
        }
        ...
        var assets = {
            ...
            processRecord = function(nodeName, node, isRow, columnMap) {
                ...
                for (typeInfo[typeIndex++] = {
                    tag: nodeName, type: "object", kind: isRow ? "row" : "record", columnMap: reverseMap(columnMap)
                }, isRow || (xml += "\r\n" + processColumnMap(columnMap)), i = 0; i < len; i++)
    ...
    var isPrivateProperty = function(nodeKey) {
            var runtimeConstants = AppMagic.Constants.Runtime,
                servicesProperties = AppMagic.Constants.Services;
            return nodeKey === runtimeConstants.collectionNameProperty || nodeKey === runtimeConstants.controlLocMapProperty || nodeKey === runtimeConstants.configurationProperty || nodeKey === runtimeConstants.metaProperty || nodeKey === runtimeConstants.syncVersionProperty || nodeKey === runtimeConstants.idProperty || nodeKey === servicesProperties.ID_PROPERTY || nodeKey === servicesProperties.SpIdProperty || nodeKey === servicesProperties.VERSION_PROPERTY ? !0 : !1
        };
    ...
    var XmlParsingStrategyBase = Core.Class.define(function XmlParsingStrategyBase_ctor(impexId) {
            ...
            _stack: null, _schemaStack: null, _schemaIdx: null, _schema: null, _strict: null, _options: null, parse: function(data) {
                    ...
                    /^ms-appdata:/g.test(text) || /^ms-appx:/g.test(text) ? this._stack.push(AppMagic.Utility.formatString(text, this._impexId)) : this._stack.push(text)
                }, _onCloseTag: function(){}, _onEnd: function() {
                    if (this._stack.length !== 1)
                        throw"Unsupported format.";
                }
        }, {}),
        Beta2XmlParsingStrategy = Core.Class.derive(XmlParsingStrategyBase, function Beta2XmlParsingStrategy_ctor(impexId) {
        ...
        Beta3XmlParsingStrategy = Core.Class.derive(XmlParsingStrategyBase, function Beta3XmlParsingStrategy_ctor(impexId) {
            XmlParsingStrategyBase.call(this, impexId)
        }, {
            parse: function(data) {
    ...
    Core.Namespace.define("AppMagic.Data", {
