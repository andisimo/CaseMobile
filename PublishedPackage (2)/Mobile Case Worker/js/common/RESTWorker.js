...
(function(Core) {"use strict";
    ...
    function channelErrorHandler(error) {
        ...
        if (typeof error.responseText == "string") {
            ...
            failResult.message = error.status ? AppMagic.Utility.formatString(AppMagic.Strings.RESTHttpErrorReturnedFormat, error.status) : AppMagic.Strings.RESTHttpErrorReturned;
        return failResult
    }
    var RESTWorker = Core.Class.define(function RESTWorker_ctor(typesByPrefix) {
            ...
            sendHttp: function(url, method, headers, queryStringParameters, body, responseType) {
                ...
                return headers && Object.keys(headers).forEach(function(x) {
                    ...
                    }), responseType && channel.responseType(responseType), channel.send(method, typeof body == "undefined" ? null : new Blob(body))
                    ...
                    var jsonPointerArray = AppMagic.Utility.parseJsonPointer(jsonPointer);
                    var dataValue = RESTWorker.getJsonValueViaJsonPointerArray(responseAsJson, jsonPointerArray);
                ...
                }, getJsonValueViaJsonPointerArray: function(rootJsonValue, jsonPointerArray) {
                    for (var resultJsonValue = rootJsonValue, i = 0; i < jsonPointerArray.length && typeof resultJsonValue != "undefined" && resultJsonValue !== null; i++)
                    ...
                    if (node[indexOrPropertyName] !== null)
                        switch (typeDef[constants.JsonPrimitiveDefinitionKey_DType]) {
                    ...
                    if (arrayProperty !== null) {
                        ...
                        for (var i = 0, len = arrayProperty.length; i < len; i++) {
                            var itemTypeDef = typesByPrefixAndName.getTypeDefinition(typeDef[constants.JsonArrayDefinitionKey_Items]);
                            ...
                            else if (itemTypeDef[constants.TypeDefinitionKey_DefType] === constants.DefinitionType_JsonPrimitive || itemTypeDef[constants.TypeDefinitionKey_DefType] === constants.DefinitionType_JsonArray) {
                                var obj = {};
    ...
    Core.Namespace.define("AppMagic.Worker", {RESTWorker: RESTWorker})
})(WinJS);